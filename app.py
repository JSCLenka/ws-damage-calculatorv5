        ...
        for card in self.all_active_cards:
            for eff in card.effects:
                if eff.trigger == trigger_type:
                    if trigger_type in ["OnAttack", "OnPlay", "OnCancel", "OnDamageCancel", "OnDamageDealt"]:
    ...
    def execute_instructions(self, instructions, source_card):
        ...
        for inst in instructions:
            ...
            if op == "DealDamage":
                ...
                mill_amount = inst.get("mill_amount", 1)
                burn_amount = inst.get("burn_amount", 0)
                burn_times = inst.get("burn_times", 1)
                
                condition = inst.get("condition", "is_cx")  # 判定条件
                mode = inst.get("mode", "per_card")         # "per_card" or "once_if_any"
                ...
                base_soul = inst.get("soul", 2)
                print(f"⚔️ [组合技/换人追击] {source_card.name} 呼叫后援！发起额外攻击！")
                trigger_soul = self.trigger_step(source_card) 
                total_soul = base_soul + trigger_soul
                print(f"  🗡️ [追击基础] 基础{base_soul}魂 + 触发{trigger_soul}魂 = 发起 {total_soul} 点伤害！")
                self.deal_damage(total_soul, source_card=source_card)
            ...
            # ==========================================
                ...
                for target_card in targets:
                    ...
                    if new_eff_json:
                        ...
                        llimit_val = new_eff_json.get("limit")
                        # 🌟 新增：支持 AI 翻译的字典格式 limit
